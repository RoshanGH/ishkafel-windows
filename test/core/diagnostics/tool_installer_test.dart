...
void main() {
  group('国内镜像源', () {
    test('Homebrew 的 bottle 与 api 都指向清华', () {
      for (final key in ['HOMEBREW_BOTTLE_DOMAIN', 'HOMEBREW_API_DOMAIN']) {
        expect(
          InstallRecipes.ffmpeg.environment[key],
    ...
    test('跳过 brew 自更新与装后清理——用户等的是 ffmpeg，不是这两件事', () {
      ...
      expect(
        InstallRecipes.ffmpeg.environment['HOMEBREW_NO_INSTALL_CLEANUP'],
        '1',
      );
    });
    ...
    test('PyPI 新旧两个变量都给，免得因 uv 版本差异悄悄走回默认源', () {
      for (final key in ['UV_DEFAULT_INDEX', 'UV_INDEX_URL']) {
        expect(
          InstallRecipes.audioSeparator.environment[key],
          contains('mirrors.tuna.tsinghua.edu.cn'),
        );
      }
    });
    ...
    test('miaoa 走官方域名，本来就在国内，不需要也不应该换', () {
      expect(
        InstallRecipes.miaoa.command.join(' '),
  ...
  group('命令要摆给用户看', () {
    ...
    test('带空格/方括号的参数加引号，照着敲也能跑', () {
      expect(
        InstallRecipes.audioSeparator.displayCommand,
  ...
  group('跑起来之后', () {
    ...
    test('第一条是那行命令本身——让人看见到底跑了什么', () async {
      final events = await installerYielding(
        ['ok'],
        [],
        0,
      ).run(InstallRecipes.miaoa).toList();
    ...
    test('失败时把最后一行原始报文带出来，不糊成「安装失败」', () async {
      final events = await installerYielding([], [
        'curl: (28) 连接超时',
      ], 1).run(InstallRecipes.miaoa).toList();
  ...
  test('ffprobe 跟着 ffmpeg 一起装，不另开一条配方', () {
    ...
    expect(InstallRecipes.forTool('不存在的工具', operatingSystem: 'macos'), isNull);
  });
  ...
  group('Windows 配方不照搬 macOS', () {
    ...
    test('Windows 安装人声分离时不受 PATH 上旧版 Python 影响', () async {
      final recipe = InstallRecipes.forTool(
        'audio-separator',
        operatingSystem: 'windows',
      )!;
      late List<String> launchedArgs;
      final installer = ToolInstaller(
        resolve: (name) =>
            name == 'uv' ? r'C:\Users\tester\.local\bin\uv.exe' : null,
        start: (_, args, _) async {
          launchedArgs = args;
          return _FakeProcess([], [], 0);
        },
      );

      final events = await installer.run(recipe).toList();

      expect(events.last.done, isTrue);
      expect(
        launchedArgs,
        containsAllInOrder(['tool', 'install', '--python', '3.12']),
        reason: 'uv 否则会优先采用 PATH 上的 Python 3.7，依赖解析直接失败',
      );
      expect(
        launchedArgs,
        containsAllInOrder(['--with', 'audioread']),
        reason: 'audio-separator 0.47.0 会导入 audioread，但包元数据漏声明了它',
      );
    });
  });
}
