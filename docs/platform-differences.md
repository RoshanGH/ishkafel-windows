...
| 范围 | Windows 规则 | 验收重点 |
|---|---|---|
| 故障报告 | Windows 增加全局手动提交、独立诊断 exe、PowerShell 进程监测；公司 API 使用 MySQL 和 IP HTTPS | 描述/截图选填，日志与硬件自动采集；无原视频/内存上传，提交权限不能读取报告；监测异步隐藏启动并验证当前 PID 首样本，不依赖 Explorer COM，整个进程作业被终止时不保证末次退出码 |
| 新建任务选择视频 | Windows 在同一 exe 的独立 helper 进程中运行原生 IFileDialog；Mac 保留系统插件 | Shell 阻塞不占主窗口线程；主窗口可取消、关闭向导会回收，最多等待 3 分钟；强制本地起始目录，不沿用失效 MRU；仅阶段/HRESULT 入日志，临时返回文件位于配置数据目录 |
...
Windows 诊断报告支持对话框内 Ctrl+V 粘贴截图：通过原生剪贴板读取 PNG 或系统位图，
限制 4 MB、900 万像素、单边 8192 像素；仅在主动粘贴时读取，提交前不创建临时图片。
截图预览可移除，文字描述选填；无文件选择或额外粘贴按钮。
