...
- [x] 先写失败测试：Windows 剪映草稿根目录来自 `LOCALAPPDATA`，Mac 仍来自 `HOME/Movies`；缺变量时给出可执行错误而不是写入相对目录。
- [x] `JianyingWriter` 使用平台化草稿目录；Windows 程序发现检查显式环境覆盖、已知安装目录和系统 PATH 解析结果。
- [x] 工作台与编导台的“显示草稿”“打开剪映”改走可注入核心服务，按钮文案平台化，失败时向用户显示原因。
- [x] 添加架构守卫，确保两个页面不再直接出现 macOS `open`。
- [x] 目标测试转绿并提交。
